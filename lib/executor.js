...
var demoProcessor = require('./processors/demosProcessor');
...
function executeImpl(dataset, dataPacket, actionIndex, callback) {
    ...
    if (actionIndex < dataPacket.actions.length) {
        ...
        if (mostRecentError && !mostRecentError.preceededAction)
        {
            ...
            if (!mostRecentError.expected) {
                if (action.command !== 'expectError') {
                    ...
                    logExpectedError('Error expected by line ' + action.line + ' occurred:\n' + mostRecentError.error.message);
                }
            }
        }
        ...
        if (handlerFunction) {
            handlerFunction(action, dataset, function(err, result) {
                ...
                dataset.__actionResult = resultWrapper;
...
var actionHandlers = {
    ...
    callYolandaQuery: function(action, dataset, callback) {
        rainbirdProcessor.callYolandaQuery(action, dataset, callback);
    },
    ...
    expectYolandaResults: function(action, dataset, callback) {
        rainbirdProcessor.expectYolandaResults(action, dataset, callback);
    },

    allowableYolandaQuestionOrder: rainbirdProcessor.defineAllowableQuestionOrder,

    querySemanticWeb: function(action,dataset, callback) {
        demoProcessor.querySemanticWeb(callback);
    },
    querySemanticWebEnvoy: function(action, dataset, callback) {
        demoProcessor.querySemanticWebEnvoy(callback);
    },
    'bridgeQuery-nationalLanguage': function(action, dataset, callback) {
        demoProcessor.bridgeQueryLanguage(action, dataset, callback);
    },
    defineYolandaAnswers: function(action, dataset, callback) {
        rainbirdProcessor.defineYolandaAnswers(action, dataset, callback);
    }
